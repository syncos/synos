...
{
    struct CPUID* cpu = (struct CPUID*)memstck_malloc(sizeof(struct CPUID));
    if (!CPUID_enabled())
    {
        cpu->enabled = false;
        return cpu;
    ...
    pushaq();
    asm ("mov eax, 0"); // Set edi to 0 = get manufacturer
    ...
    asm ("mov eax, 1");
    asm ("cpuid");
    asm volatile ("mov esi, eax" : "=S" (eax));
    asm volatile ("mov esi, edx" : "=S" (edx));
    asm volatile ("mov esi, ecx" : "=S" (ecx));
    popaq();

    cpu->ProcessorInfoEAX = eax;
    cpu->ProcessorInfoEDX = edx;
    cpu->ProcessorInfoECX = ecx;
